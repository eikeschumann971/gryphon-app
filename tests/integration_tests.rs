...
async fn test_in_memory_event_store() {
    ...
    // Test append
    event_store.append_events("test-aggregate", 0, vec![event_envelope.clone()]).await.unwrap();
...
async fn test_in_memory_snapshot_store() {
    ...
    // Test save
    snapshot_store.save_snapshot(snapshot.clone()).await.unwrap();
    
    // Test load
    let loaded_snapshot = snapshot_store.load_snapshot("test-aggregate", None).await.unwrap();
