...
mod path_planner_tests {
    ...
    fn test_worker_registration_duplicate_worker() {
        ...
        planner.register_worker(worker_id.clone(), capabilities.clone()).unwrap();
    ...
    fn test_path_plan_request_valid_positions() {
        ...
        assert_eq!(planner.uncommitted_events().len(), 2);
        match &planner.uncommitted_events()[1] {
            PathPlanningEvent::PathPlanRequested { request_id, agent_id, .. } => {
    ...
    fn test_worker_ready_success() {
        ...
        planner.register_worker(worker_id.clone(), vec![PlanningAlgorithm::AStar]).unwrap();
        ...
        match &planner.uncommitted_events()[2] {
            PathPlanningEvent::WorkerReady { worker_id: event_worker_id, .. } => {
                assert_eq!(*event_worker_id, worker_id);
            }
    ...
    fn test_plan_assignment_with_idle_worker() {
        ...
        assert_eq!(worker.current_plan_id.as_ref().unwrap(), &planner.active_plans[0].id);
    }
    ...
    fn test_plan_assignment_acceptance() {
        ...
        assert!(events.iter().any(|e| matches!(e, PathPlanningEvent::PlanAssignmentAccepted { .. })));
    }
    ...
    fn test_plan_completion() {
        ...
        planner.request_path_plan(request).unwrap();
        let plan_id = planner.active_plans[0].id.clone();
        planner.handle_plan_assignment_accepted(worker_id.clone(), plan_id.clone()).unwrap();
        ...
        let result = planner.handle_plan_completed(worker_id.clone(), plan_id.clone(), waypoints.clone());
        assert!(result.is_ok());
        ...
        let events = planner.uncommitted_events();
        assert!(events.iter().any(|e| matches!(e, PathPlanningEvent::PlanCompleted { .. })));
        assert!(events.iter().any(|e| matches!(e, PathPlanningEvent::WorkerReady { .. })));
    }
    ...
    fn test_plan_failure() {
        ...
        let result = planner.handle_plan_failed(worker_id.clone(), plan_id.clone(), failure_reason.clone());
        assert!(result.is_ok());
        ...
        // Check events were emitted
        let events = planner.uncommitted_events();
        assert!(events.iter().any(|e| matches!(e, PathPlanningEvent::PlanFailed { .. })));
        assert!(events.iter().any(|e| matches!(e, PathPlanningEvent::WorkerReady { .. })));
    }
    ...
    fn test_multiple_workers_assignment_priority() {
        ...
        planner.register_worker("worker-1".to_string(), vec![PlanningAlgorithm::AStar]).unwrap();
        planner.register_worker("worker-2".to_string(), vec![PlanningAlgorithm::Dijkstra]).unwrap();
        planner.register_worker("worker-3".to_string(), vec![PlanningAlgorithm::AStar, PlanningAlgorithm::Dijkstra]).unwrap();
        ...
        let worker2 = planner.registered_workers.iter().find(|w| w.worker_id == "worker-2").unwrap();
        let worker3 = planner.registered_workers.iter().find(|w| w.worker_id == "worker-3").unwrap();
    ...
    fn test_path_planning_events_serialization() {
        let events = vec![
            ...
            PathPlanningEvent::PlanCompleted {
                ...
                waypoints: vec![Position2D { x: 5.0, y: 5.0 }, Position2D { x: 10.0, y: 10.0 }],
                timestamp: Utc::now(),
            },
        ];
        ...
        for event in events {
            ...
            assert_eq!(std::mem::discriminant(&event), std::mem::discriminant(&deserialized));
        }
    }
