...
async fn pgstore_persist_and_publish() {
    ...
    let store = build_pg_store_with_bus::<PathPlanner, _>(&url, bus_clone).await.expect("store built");
    ...
    let events = PathPlanner::handle_command(agg_state.inner(), gryphon_app::esrs::path_planning::PathPlannerCommand::CreatePlanner { planner_id: agg_state.id().to_string(), algorithm: gryphon_app::domains::path_planning::aggregate::types::PlanningAlgorithm::AStar }).expect("handle");

    // Persist using store.persist API (accepts &mut AggregateState)
    store.persist(&mut agg_state, events).await.expect("persist");
